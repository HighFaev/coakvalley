...
for i in range(9):
    match i:
        ...
        case 4:
            # Башня мага - маг
            quests.append(Quest(i, make_special_symbols(open("tasks/text/" + str(i + 1) + ".txt", "r+").read()), 31.6 * vh, 46.5 * vw))
        case 5:
            # Утес - маг
            quests.append(Quest(i, make_special_symbols(open("tasks/text/" + str(i + 1) + ".txt", "r+").read()), 18 * vh, 15 * vw))
        case 6:
            # Главный штаб - генерал
            quests.append(Quest(i, make_special_symbols(open("tasks/text/" + str(i + 1) + ".txt", "r+").read()), 28.2 * vh, 54 * vw))
        case 7:
            # Карчма - генерал
            quests.append(Quest(i, make_special_symbols(open("tasks/text/" + str(i + 1) + ".txt", "r+").read()), 71.5 * vh, 38.7 * vw))
        case 8:
            # Дворец - королева
            quests.append(Quest(i, make_special_symbols(open("tasks/text/" + str(i + 1) + ".txt", "r+").read()), 40 * vh, 59 * vw))
...
def make_dialog_window(cur_line):
    ...
    if cur_line >= 1000000:
        ...
        if parametrs[0] != 0:
            player_save["lastlevel"] = parametrs[0]
